        ...
        row.innerHTML = `
            ...
            <td>
                <div class="action-buttons">
                    <button class="btn btn-small" onclick="verDetalles(${notif.id_notificacion})" title="Ver detalles">👁️ Ver</button>
                    ...
                    <button class="btn btn-small btn-danger" onclick="eliminarNotificacion(${notif.id_notificacion})" title="Eliminar">🗑️</button>
                </div>
            </td>
        `;
...
notificacionForm.addEventListener('submit', function(e) {
    ...
    if (fechaEnvio < hoy) {
        showAlert('⚠️ La fecha de envío no puede ser anterior a hoy', 'error');
        return;
    }
...
window.programarNotificacion = function() {
    ...
    if (!fechaInput.value || !horaInput.value) {
        showAlert('⚠️ Para programar una notificación debes especificar fecha y hora', 'error');
        return;
    }
