...
function loadUserData() {
    ...
    document.getElementById('securityCode').textContent = userData.codigoSeguridad;
...
function renovarCarnet() {
    ...
    setTimeout(() => {
        showNotification('Proceso de renovación iniciado. Recibirás un email con los pasos a seguir.', 'success');
    }, 2000);
}

function reportarPerdida() {
    if (confirm('¿Estás seguro de que deseas reportar la pérdida o daño de tu carnet de amenidades?')) {
        showNotification('Reportando problema...', 'warning');
        
        setTimeout(() => {
            showNotification('Problema reportado. El acceso a amenidades será desactivado temporalmente. Contacta administración para obtener un nuevo carnet.', 'warning');
        }, 2000);
    }
}
...
function validateCarnetStatus() {
    ...
    if (!isValid) {
        statusIndicator.innerHTML = '<span class="estado-vencido">CARNET VENCIDO</span>';
